...
export function AgentForm({ agent }: { agent?: Agent }) {
  ...
  const voiceValue = agent?.voice_id || "gemini-natural-female";
  const voiceLabel = voiceValue === "gemini-natural-female" ? "Natural female voice" : "Saved workspace voice";
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <CardTitle>{agent ? "Edit voice agent" : "Create voice agent"}</CardTitle>
              <CardDescription>Describe the business, the opening line, and the outcome callers should reach.</CardDescription>
            </div>
            <Button type="button" variant="outline" onClick={improveAgent} disabled={isImproving || isPending}>
              <Sparkles aria-hidden="true" />
              {isImproving ? "Making clearer..." : "Make clearer"}
            </Button>
          </div>
        </CardHeader>
        <CardContent>
          <FieldGroup>
            <div className="grid gap-4 lg:grid-cols-2">
              <Field>
                ...
                <FieldDescription>Shown in the dashboard and call history.</FieldDescription>
              </Field>
              <Field>
                <FieldLabel htmlFor="language_display">Caller languages</FieldLabel>
                <Input id="language_display" value="Detect languages automatically" readOnly />
                <input type="hidden" name="language" value="multilingual-IN" />
                <FieldDescription>Works with English, Malayalam, Tamil, Telugu, Kannada, Hindi, and mixed speech.</FieldDescription>
              </Field>
            </div>
            <Field>
              <FieldLabel htmlFor="description">What this business does</FieldLabel>
              <Input id="description" name="description" defaultValue={agent?.description || ""} />
            </Field>
            <Field>
              <FieldLabel htmlFor="system_prompt">What the agent should know</FieldLabel>
              <Textarea
                id="system_prompt"
                name="system_prompt"
                className="min-h-44"
                ...
                required
              />
              <FieldDescription>Add products, prices, common questions, lead rules, and anything the agent should say or avoid.</FieldDescription>
            </Field>
            <Field>
              <FieldLabel htmlFor="first_message">Opening message</FieldLabel>
              <Input
                id="first_message"
                name="first_message"
            ...
            <details className="rounded-md border p-4">
              <summary className="cursor-pointer text-sm font-medium">Call behavior and routing</summary>
              <div className="mt-4 grid gap-4">
                <div className="grid gap-4 lg:grid-cols-3">
                  <Field>
                    <FieldLabel htmlFor="voice_label">Voice style</FieldLabel>
                    <Input id="voice_label" value={voiceLabel} readOnly />
                    <input type="hidden" name="voice_id" defaultValue={voiceValue} />
                    <FieldDescription>Voice changes can be handled during onboarding.</FieldDescription>
                  </Field>
                  <Field>
                    <FieldLabel htmlFor="temperature">Answer variety</FieldLabel>
                    <Input
                      id="temperature"
                      name="temperature"
                      type="number"
                  ...
                  <Field>
                    <FieldLabel htmlFor="max_call_duration_seconds">Maximum call length</FieldLabel>
                    <Input
                    ...
                    />
                    <FieldDescription>Usually 300 to 900 seconds for business calls.</FieldDescription>
                  </Field>
                </div>
                <div className="grid gap-4 lg:grid-cols-2">
                  <Field>
                    <FieldLabel htmlFor="silence_timeout_ms">Pause before replying</FieldLabel>
                    <Input
                    ...
                    />
                    <FieldDescription>Higher values give callers a little more time to finish speaking.</FieldDescription>
                  </Field>
                  <Field>
                    <FieldLabel htmlFor="vobiz_phone_number">Phone number</FieldLabel>
                    <Input
                ...
                <Field>
                  <FieldLabel htmlFor="vobiz_sip_config">Call routing notes</FieldLabel>
                  <Textarea
                    id="vobiz_sip_config"
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  <FieldSet>
                    <FieldLegend>Operating preference</FieldLegend>
                    <ToggleGroup value={costMode} onValueChange={(value) => value && setCostMode(value as CostMode)} name="cost mode">
                      <ToggleGroupItem value="economy">Lower cost</ToggleGroupItem>
                      <ToggleGroupItem value="balanced">Balanced</ToggleGroupItem>
                      <ToggleGroupItem value="quality">Best answers</ToggleGroupItem>
                    </ToggleGroup>
                  </FieldSet>
                  <FieldSet>
                    <FieldLegend>Reply speed</FieldLegend>
                    <ToggleGroup value={latencyMode} onValueChange={(value) => value && setLatencyMode(value as LatencyMode)} name="latency mode">
                      <ToggleGroupItem value="ultra-low">Fastest</ToggleGroupItem>
                      <ToggleGroupItem value="balanced">Balanced</ToggleGroupItem>
                      <ToggleGroupItem value="quality">Best answers</ToggleGroupItem>
                    </ToggleGroup>
                  </FieldSet>
